...
fn serialize_color<S>(input: &[u8; 4], serializer: S) -> Result<S::Ok, S::Error> 
	where S: Serializer 
{
	let input = ColorDetailed {
		r: input[0],
		g: input[1],
		b: input[2],
		a: input[3],
...
{
	let de = ColorDetailed::deserialize(deserializer)?;
	Ok([de.r, de.g, de.b, de.a])
}
